...
int MnemonicsX64::getInstructionIndex(std::string instructionMnemonic)
{
	...
	for (int i = 0; i < instructionMnemonicsSize; i++)
	{
		if (((int)INSTRUCTION_MNEMONICS[i] < 65 || (int)INSTRUCTION_MNEMONICS[i] >122) && (int)INSTRUCTION_MNEMONICS[i] > 0)
		{
			...
			strncpy(temp, INSTRUCTION_MNEMONICS + startIndex + 1, offset + 1);
			temp[offset] = '\0';
...
int MnemonicsX64::getInstructionArgument(std::string instructionPart, InstructionArgument* instructionArgument)
{
	...
	if (startBracketIndex0 != std::string::npos && endBracketIndex0 != std::string::npos)
	{
		...
		if (plusIndex != std::string::npos)
		{
			...
			sscanf(offsetValue.c_str(), "0X%I64X", &instructionArgument->offsetAddress);
			instructionArgument->registerIndex = registerIndex;
			strcpy((char*)instructionArgument->registerMnemonic.c_str(), registerMnemonic.c_str());
	...
	else
	{
		...
		else
		{
			...
			sscanf(instructionPart.c_str(), "0X%I64X", &instructionArgument->value);
